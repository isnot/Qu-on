...
const flickrImageUrl = async (latlon) => {
  const flickr_key = 'f8c30ae7ac87e73ad0f516e314b5cfef';
  const url = `https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=${flickr_key}&privacy_filter=1&content_type=1&media=photos&lat=${latlon[1]}&lon=${latlon[0]}&radius=0.1&format=json&nojsoncallback=1`;
  try {
...
map.on('load', () => {
  ...
  config.chapters.forEach((record, idx) => {
    ...
    (async () => {
      ...
      const imageUrl = await flickrImageUrl(record.location.center);
      if (imageUrl) {
